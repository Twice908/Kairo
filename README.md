...
The `app` module is organized by package boundaries under `com.kairo.player`: `audio`, `playback`, `source`, `data`, `domain`, `network`, `cache`, `di`, `ui`, and `util`. A Hilt singleton owns the long-lived ExoPlayer instance. Media3 input-format and decoder callbacks feed nullable audio diagnostics; unavailable source values are left unknown. Jetpack Compose, Hilt, and Room remain available for UI, dependency injection, and local persistence.
